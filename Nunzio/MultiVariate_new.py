...
def prepare_data_for_chronos(dataset_path: str, context_length: int = 100, prediction_length: int = 64):
    ...
    # Build all at once to avoid DataFrame fragmentation warning
    df_chronos = pd.concat([
        pd.DataFrame({'timestamp': pd.date_range(start="2026-01-01 00:00:00", periods=len(df_clean), freq="min")}),
